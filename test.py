...
# Case 2: Check if UnitPrice can be converted back to original USD value
usd_to_thb_rate = 33.77
mergedTracks = pd.merge(tracksInput, tracksOutput, on='TrackId', suffixes=('_input', '_output'))
#mergedTracks['ReversedUnitPrice'] = (mergedTracks['UnitPrice_output'] / usd_to_thb_rate).round(2)

if (mergedTracks['UnitPrice_output'] == (mergedTracks['UnitPrice_input'] * usd_to_thb_rate)).all():
    f.write("Case 2: Pass\n")
else:
    f.write("Case 2: Fail\n")    
